...
from collocator import CollocationFinder, DependencyLevelCollocationFinder, SentenceLevelCollocationFinder
...
def get_sentences(language, keep_dependencies=False):
	if language in ('en', 'cz'):
		import czeng
		corpus_directory = os.path.join('data', 'czeng')
		sentences = czeng.open_filtered_files(corpus_directory, language, keep_dependencies=keep_dependencies, verbose=True)
	else:
		raise Exception('Unknown language \'{}\'.'.format(language))
	
	return sentences

def get_collocation_finder(language, sentence_level=False, dependency_level=False):
	...
	if os.path.isfile(collocations_filename):
		...
		sentences = get_sentences(language, keep_dependencies=dependency_level)
		lexicon_filename = os.path.join('data', 'lexicons', 'original_trimmed_' + language + '.txt')
		lexicon = load_lexicon(lexicon_filename)
		collocator = Collocator(sentences, lexicon, verbose=True)
...
def test(args):
	...
	model = get_collocation_finder(language, dependency_based=args.dependency_based)
