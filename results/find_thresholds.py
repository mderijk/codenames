...
def analyze_dep_rel_labels(args, version='v1.0'):
	...
	games = get_games('data_2019-10-30-used-for-thresholds', exclude_blacklisted_games=False)
	decision_ratings_by_ai = defaultdict(list)
	number_of_games_by_ai = Counter()
	for game in games:
		ai_name = game['ai_name']
		if version in ai_name and '_{}_'.format(language) in ai_name and method in ai_name and 'top' in ai_name:
			number_of_games_by_ai[ai_name] += 1
			
			hinted_at = set()
			for index, ((hint, overall_rating), decisions, board) in enumerate(zip(game['hints'], game['decisions'], game['hints_board'])):
				...
				top_n = 3 if 'top_3' in ai_name else (2 if 'top_2' in ai_name else 1)
